...
from .utils import either_dict_or_kwargs, is_dict_like

from typing import Iterable

# from pint import DimensionalityError, UndefinedUnitError, units


def _interpret_kwargs(self, look_for, **kwargs):
    present = set(look_for) & set(kwargs)

    for kw in present:
        axis = kwargs[kw]
        if axis in axis_names:
            kwargs[kw] = da.cf._axis_name(axis)

            return kwargs


__all__ = []
axis_names = ["T", "Z", "Y", "latitude", "X", "longitude"]
...
coordinate_criteria = {
    ...
    "units": {
        "Z": {"match": "dimensionality", "units": "Pa"},
        "latitude": {
            "match": "name",
            "units": {
                "degree_north",
...
class CFDataArrayAccessor:
    ...
    def plot(self, *args, **kwargs):
        self._data_array.plot(*args, **kwargs)
    ...
    def _fixup_coordinate_map(self, coord_map):
        ...
        for axis in coord_map:
            if coord_map[axis] is not None and not isinstance(
                coord_map[axis], xr.DataArray
            ):
    ...
    def _generate_coordinate_map(self):
        ...
        #     coord_lists[axis] = [
        #         coord for coord in coord_lists[axis] if coord.ndim <= 1
        #     ]
    ...
    def _resolve_axis_duplicates(self, axis, coord_lists):
        ...
        dimension_coords = [
            coord_var
            for coord_var in coord_lists[axis]
            if coord_var.name in coord_var.dims
        ]
        ...
        varname = (
            ...
            if self._data_array.name is not None
            else ""
        )
        warnings.warn(
    ...
    def _metpy_axis_search(self, metpy_axis):
        ...
        for axis, coord_var in coord_map.items():
            if coord_var is not None and not any(
                axis in coord.attrs.get("_metpy_axis", "").split(",")
                for coord in coords
            ):
    ...
    def _axis_name(self, axis):
        ...
        if axis in axis_names:
            coord_var = self._metpy_axis_search(axis).name
            if coord_var is not None:
                return coord_var
            else:
                raise AttributeError(axis + " attribute is not available.")
    ...
    def coordinates_identical(self, other):
        ...
        for coord_name, coord_var in self._data_array.coords.items():
            if coord_name not in other.coords or not other[coord_name].identical(
                coord_var
            ):
                return False
    ...
    def find_axis_name(self, axis):
        ...
        elif axis not in self._data_array.dims and axis in axis_names:
    ...
    def assign_latitude_longitude(self, force=False):
        ...
        new_dataarray = self._data_array.assign_coords(
...
class CFDatasetAccessor:
    ...
    def parse_cf(self, varname=None, coordinates=None):
        ...
        if crs is None and not check_axis(var, "latitude", "longitude"):
            ...
            def _has_coord(coord_type):
                return any(
                    check_axis(coord_var, coord_type)
                ...
                try:
                    ...
                    if crs is not None:
                        ...
                        new_coord_var = coord_var.copy(
                            data=(
                                coord_var.metpy.unit_array * (height * units.meter)
                            ).m_as("meter")
                        )
    ...
    def assign_latitude_longitude(self, force=False):
        ...
        for data_var in self._dataset.data_vars.values():
            if hasattr(data_var.metpy, "y") and hasattr(data_var.metpy, "x"):
                ...
                if not force and (
                    hasattr(data_var.metpy, "latitude")
    ...
    def assign_y_x(self, force=False, tolerance=None):
        ...
        grid_prototype = None
        for data_var in self._dataset.data_vars.values():
            if hasattr(data_var.metpy, "latitude") and hasattr(
                data_var.metpy, "longitude"
            ):
                if grid_prototype is None:
                    grid_prototype = data_var
                if not force and (
...
def _build_y_x(da, tolerance):
    ...
    if latitude.dims != longitude.dims:
        ...
        raise ValueError(
            "To build 1D y/x coordinates via assign_y_x, latitude/longitude "
            "must be 2D"
        )

    # Convert to projected y/x
    xxyy = da.metpy.cartopy_crs.transform_points(
        ccrs.Geodetic(da.metpy.cartopy_globe), longitude.values, latitude.values
    )
...
def preprocess_xarray(func):
    ...
    def wrapper(*args, **kwargs):
        args = tuple(
            a.metpy.unit_array if isinstance(a, xr.DataArray) else a for a in args
        )
        kwargs = {
            name: (v.metpy.unit_array if isinstance(v, xr.DataArray) else v)
        ...
        if len(data_arrays) > 1:
            ...
            for other in data_arrays[1:]:
                if not first.metpy.coordinates_identical(other):
                    raise ValueError(
                        "Input DataArray arguments must be on same coordinates."
                    )
        return func(*args, **kwargs)
...
def _reassign_quantity_indexer(data, indexers):
    ...
    for coord_name in indexers:
        ...
        if isinstance(indexers[coord_name], slice):
            ...
            # Handle quantities
            reassigned_indexers[coord_name] = _to_magnitude(
                indexers[coord_name], coord_units
            )
